...
pub fn solve_maze(mut maze: &mut ProcessedMaze) {
    let starting_node = maze.starting_node;
    search_path(maze, starting_node);
}
...
pub fn search_path(mut maze: &mut ProcessedMaze, node_id: u32) {
    ...
    let node = &mut &maze.maze_nodes[node_id as usize];
    if node.left_wall == true && node.num_of_openings < 3 {
        ...
        search_path(&mut maze, move_left.node_id);
    }
}
